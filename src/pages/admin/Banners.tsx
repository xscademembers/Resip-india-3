    ...
    <>
      ...
      {loading ? (
        ...
        >
          {banners.map((b) => (
            <tr key={b._id}>
              <Td>{b.image && <img src={b.image} alt="" className="h-10 w-20 rounded object-cover" loading="lazy" />}</Td>
              <Td className="font-semibold text-charcoal">{b.title || '(untitled)'}</Td>
              <Td className="text-charcoal/60">{b.position}</Td>
              <Td>{b.sortOrder}</Td>
              <Td>
                <span className={`rounded-full px-2 py-1 text-xs font-bold ${b.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                  {b.isActive ? 'Active' : 'Inactive'}
                </span>
              </Td>
              <Td className="text-right">
                <div className="flex justify-end gap-1">
                  <button onClick={() => openEdit(b)} className="rounded-lg p-2 text-brand-blue hover:bg-brand-blue/5" aria-label="Edit">
                    <Pencil size={16} />
                  </button>
                  <button onClick={() => remove(b._id)} className="rounded-lg p-2 text-red-500 hover:bg-red-50" aria-label="Delete">
                    <Trash2 size={16} />
                  </button>
                </div>
              </Td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Edit Banner' : 'Add Banner'}>
        <form onSubmit={save}>
          ...
          <Field label="Link">
            <input className={inputClass} value={form.link} onChange={(e) => setForm({ ...form, link: e.target.value })} placeholder="/shop" />
          </Field>
